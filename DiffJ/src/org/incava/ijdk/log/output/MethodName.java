...
public class MethodName extends Item {
	public MethodName(ANSIColor color, StackElements stackElements,
			int methodWidth) {
	...
	public boolean isRepeated(StackElements stackElements) {
		...
		return (ObjectExt.areEqual(previous.getMethodName(),
				current.getMethodName()) && ObjectExt.areEqual(
				previous.getClassName(), current.getClassName()));
	}
