...
public abstract class NonBooleanOption<VarType> extends Option<VarType> {
	...
	public NonBooleanOption(String longName, String description,
			Character shortName, VarType value) {
	...
	public boolean set(String arg, List<String> argList) throws OptionException {
		return setFromLongName(arg, argList) || setFromLongNameEq(arg)
				|| setFromShortName(arg, argList);
	}

	protected void checkArgList(Object name, List<String> argList)
			throws InvalidTypeException {
		if (argList.isEmpty()) {
			throw new InvalidTypeException(name + " expects following "
					+ getType() + " argument");
		}
	}

	protected void checkArgString(Object name, String arg, int pos)
			throws InvalidTypeException {
		if (pos >= arg.length()) {
			throw new InvalidTypeException(name + " expects argument of type "
					+ getType());
		}
	}

	protected boolean setFromLongName(String arg, List<String> argList)
			throws OptionException {
		String longName = getLongName();
		return arg.equals("--" + longName) ? setFromArgList(longName, argList)
				: false;
	}

	protected boolean setFromLongNameEq(String arg) throws OptionException {
		String longNameEq = "--" + getLongName() + "=";
		return arg.startsWith(longNameEq) ? setFromStringPosition(
				getLongName(), arg, longNameEq.length()) : false;
	}

	protected boolean setFromStringPosition(String name, String arg, int pos)
			throws InvalidTypeException {
	...
	protected boolean setFromShortName(String arg, List<String> argList)
			throws OptionException {
		Character shortName = getShortName();
		return shortName != null && arg.equals("-" + shortName) ? setFromArgList(
				shortName, argList) : false;
	}

	protected boolean setFromArgList(Object name, List<String> argList)
			throws OptionException {
