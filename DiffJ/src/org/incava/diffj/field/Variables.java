...
public class Variables {
	public static final Message VARIABLE_REMOVED = new Message(
			"variable removed: {0}", ChangeType.VARIABLE_REMOVED);
	public static final Message VARIABLE_ADDED = new Message(
			"variable added: {0}", ChangeType.VARIABLE_ADDED);
	public static final Message VARIABLE_CHANGED = new Message(
			"variable changed from {0} to {1}", ChangeType.VARIABLE_CHANGED);
	...
	public void diff(Variables toVariables, Differences differences) {
		...
		Map<String, ASTVariableDeclarator> toNamesToDecls = toVariables
				.getNamesToDeclarations();
		...
		for (String name : names) {
			...
			if (fromVarDecl != null && toVarDecl != null) {
				...
				processAddDelVariable(name, VARIABLE_ADDED, variables.get(0),
						toVarDecl, differences);
			} else {
				processAddDelVariable(name, VARIABLE_REMOVED, fromVarDecl,
						toVariables.variables.get(0), differences);
			}
		}
	}
	...
	protected void processAddDelVariable(String name, Message msg,
			ASTVariableDeclarator fromVarDecl, ASTVariableDeclarator toVarDecl,
