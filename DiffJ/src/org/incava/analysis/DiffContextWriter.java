...
public class DiffContextWriter extends DiffWriter {
	...
	protected String getLine(List<String> lines, int lidx, int fromLine,
			int fromColumn, int toLine, int toColumn, boolean isDelete) {
		StringBuilder sb = new StringBuilder();
		sb.append("! ").append(lines.get(lidx - 1)).append(EOLN);
		return sb.toString();
	}

	protected String printLines(StringBuilder sb, boolean isDelete,
			FileDiff fdiff, LocationRange loc, List<String> lines) {
		...
		for (int lidx = fromLine; lidx <= toLine; ++lidx) {
			String line = getLine(lines, lidx, fromLine, fromColumn, toLine,
					toColumn, isDelete);
			sb.append(line);
		}
