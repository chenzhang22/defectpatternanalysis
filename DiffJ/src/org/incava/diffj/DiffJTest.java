...
public class DiffJTest extends IncavaTestCase {
	...
	public Location loc(Location loc, String var) {
		return loc(loc.getLine(),
				loc.getColumn() + (var == null ? 0 : var.length() - 1));
	}

	public LocationRange locrg(int fromLine, int fromColumn, int toLine,
			int toColumn) {
		return new LocationRange(loc(fromLine, fromColumn), loc(toLine,
				toColumn));
	}
	...
	public String[] getOutput(String fromLines, String toLines) {
		...
		evaluate(fromLines, toLines, Java.SOURCE_1_5, // source
				makeDetailedReport(writer), (FileDiff[]) null); // no code
																// expectation
																// comparisons
	...
	public void evaluate(Lines fromLines, Lines toLines,
			FileDiff... expectations) {
		evaluate(fromLines.toString(), toLines.toString(), expectations);
	}

	public void evaluate(Lines fromLines, Lines toLines, String src,
			FileDiff... expectations) {
		evaluate(fromLines.toString(), toLines.toString(), src, expectations);
	}

	public void evaluate(Lines fromLines, Lines toLines, String src,
			Report report, FileDiff... expectations) {
		evaluate(fromLines.toString(), toLines.toString(), src, report,
				expectations);
	}
	...
	public void evaluate(String from, String to, String src,
			FileDiff... expectations) {
		evaluate(from, to, src, makeReport(new StringWriter()), expectations);
	}

	public void evaluate(String fromStr, String toStr, String src,
			Report report, FileDiff... expectations) {
		evaluate("-", fromStr, "-", toStr, src, report, expectations);
	}

	public void evaluate(String fromName, String fromStr, String toName,
			String toStr, String src, Report report, FileDiff... expectations) {
		assertDiffsEqual(fromName, fromStr, toName, toStr, src, report,
				expectations);
	}

	public void assertDiffsEqual(String fromName, String fromStr,
			String toName, String toStr, String src, Report report,
			FileDiff... expectations) {
	...
	public void assertDiffsEqual(JavaFile fromFile, JavaFile toFile,
			Report report, FileDiff... expectations) {
	...
	public void assertOutputEquals(Lines fromLines, Lines toLines,
	...
	public void assertDifferencesEqual(FileDiff[] expectedDiffs,
			Collection<FileDiff> actualDiffs) {
		if (expectedDiffs.length != actualDiffs.size()) {
			...
			assertEquals("number of differences", expectedDiffs.length,
					actualDiffs.size());
		}
	...
	public void assertDifferenceEqual(FileDiff expectedDiff,
			FileDiff actualDiff, int di) {
