...
public class Tkn {
	...
	public Location getBeginLocation() {
		return token == null ? null : new Location(token.beginLine,
				token.beginColumn);
	}
	...
	public Location getEndLocation() {
		return token == null ? null : new Location(token.endLine,
				token.endColumn);
	}
	...
	public int compareTo(Tkn other) {
		...
		int cmp = new Integer(token.kind).compareTo(new Integer(
				other.token.kind));
		if (cmp == 0) {
