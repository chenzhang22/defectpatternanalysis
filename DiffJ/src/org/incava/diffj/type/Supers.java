...
public abstract class Supers {
	...
	protected void superTypeAdded(ASTClassOrInterfaceType toType,
			String typeName, Differences differences) {
		differences.changed(decl, toType, getAddedMessage(), typeName);
	}

	protected void superTypeChanged(ASTClassOrInterfaceType fromType,
			String fromName, ASTClassOrInterfaceType toType, String toName,
			Differences differences) {
		differences.changed(fromType, toType, getChangedMessage(), fromName,
				toName);
	}

	protected void superTypeRemoved(ASTClassOrInterfaceType fromType,
			String typeName, Differences differences) {
	...
	public void diff(Supers toSupers, Differences differences) {
		...
		if (fromMap.hasOne() && toMap.hasOne()) {
			// I don't like this special case, but it is better than two
			// separate
	...
	protected void compareOne(NameToDecl fromMap, NameToDecl toMap,
			Differences differences) {
	...
	protected void compareEach(NameToDecl fromMap, NameToDecl toMap,
			Supers toSupers, Differences differences) {
