...
public class Throws {
	public static final Message THROWS_REMOVED = new Message(
			"throws removed: {0}", ChangeType.THROWS_REMOVED);
	public static final Message THROWS_ADDED = new Message("throws added: {0}",
			ChangeType.THROWS_ADDED);
	public static final Message THROWS_REORDERED = new Message(
			"throws {0} reordered from argument {1} to {2}",
			ChangeType.THROWS_REORDERED);
	...
	protected void changeThrows(SimpleNode fromNode, SimpleNode toNode,
			Message msg, ASTName name, Differences differences) {
		differences.changed(fromNode, toNode, msg,
				SimpleNodeUtil.toString(name));
	}
	...
	protected void compareEachThrow(Throws toThrows, Differences differences) {
		...
		for (int fromIdx = 0; fromIdx < fromNames.size(); ++fromIdx) {
			...
			if (throwsMatch == null) {
				changeThrows(fromName, toThrows.nameList, THROWS_REMOVED,
						fromName, differences);
				...
				differences.changed(fromName, toName, THROWS_REORDERED,
						fromNameStr, fromIdx, throwsMatch);
			}
		}
		...
		for (int toIdx = 0; toIdx < toNames.size(); ++toIdx) {
			if (toNames.get(toIdx) != null) {
				...
				changeThrows(nameList, toName, THROWS_ADDED, toName,
						differences);
			}
		}
	}

	protected Integer getMatch(List<ASTName> fromNames, int fromIdx,
			List<ASTName> toNames) {
		String fromNameStr = SimpleNodeUtil.toString(fromNames.get(fromIdx));

		for (int toIdx = 0; toIdx < toNames.size(); ++toIdx) {
			if (toNames.get(toIdx) != null
					&& SimpleNodeUtil.toString(toNames.get(toIdx)).equals(
							fromNameStr)) {
