...
public class Filter {
	...
	public Filter(Level level, Pattern fnamePat, Range lnum, Pattern clsName,
			Pattern methName) {
		this.level = level;
	...
	public Filter(Level level, String fname, Range lnum, String clsName,
			String methName) {
		this(level, toPattern(fname), lnum, toPattern(clsName),
				toPattern(methName));
	}
	 ...
	 */
	public boolean isMatch(String fileName, int lineNumber, String className,
			String methodName) {
		return (isMatch(fileNamePat, fileName)
				&& isMatch(lineNumberRng, lineNumber)
				&& isMatch(classNamePat, className) && isMatch(methodNamePat,
					methodName));
	}
	...
	public boolean isMatch(StackTraceElement ste) {
		return isMatch(ste.getFileName(), ste.getLineNumber(),
