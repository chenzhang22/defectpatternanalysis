...
public class ParameterTypeMatch extends ParameterMatch {
	public ParameterTypeMatch(ASTFormalParameter fromFormalParam, int index,
	...
	public void diff(Differences differences) {
		...
		if (fromNameTk.image.equals(toNameTk.image)) {
			differences.changed(fromNameTk, toNameTk,
					Parameters.PARAMETER_REORDERED, fromNameTk.image, index,
					toIdx);
		} else {
			differences.changed(fromNameTk, toNameTk,
					Parameters.PARAMETER_REORDERED_AND_RENAMED,
					fromNameTk.image, index, toIdx, toNameTk.image);
		}
	}
}
