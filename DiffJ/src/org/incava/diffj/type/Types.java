...
public class Types {
	public static final Message TYPE_DECLARATION_ADDED = new Message(
			"type declaration added: {0}", ChangeType.TYPE_DECLARATION_ADDED);
	public static final Message TYPE_DECLARATION_REMOVED = new Message(
			"type declaration removed: {0}",
			ChangeType.TYPE_DECLARATION_REMOVED);
	...
	protected void compareTypes(ASTTypeDeclaration fromTypeDecl,
			ASTTypeDeclaration toTypeDecl, Types toTypes,
			Differences differences) {
		if (fromTypeDecl == null) {
			Token toName = TypeDeclarationUtil.getName(toTypeDecl);
			differences.added(compUnit, toTypeDecl, TYPE_DECLARATION_ADDED,
					toName.image);
		} else if (toTypeDecl == null) {
			Token toName = TypeDeclarationUtil.getName(fromTypeDecl);
			differences.deleted(fromTypeDecl, toTypes.compUnit,
					TYPE_DECLARATION_REMOVED, toName.image);
		} else {
			ASTClassOrInterfaceDeclaration fromDecl = TypeDeclarationUtil
					.getType(fromTypeDecl);
			ASTClassOrInterfaceDeclaration toDecl = TypeDeclarationUtil
					.getType(toTypeDecl);
