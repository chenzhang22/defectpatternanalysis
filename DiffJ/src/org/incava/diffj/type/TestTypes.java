...
public class TestTypes extends ItemsTest {
	...
	protected FileDiff makeTypeAddedRef(LocationRange fromLoc,
			LocationRange toLoc, String added) {
		return new FileDiffAdd(fromLoc, toLoc, Types.TYPE_DECLARATION_ADDED,
				added);
	}

	protected FileDiff makeTypeRemovedRef(LocationRange fromLoc,
			LocationRange toLoc, String removed) {
		return new FileDiffDelete(fromLoc, toLoc,
				Types.TYPE_DECLARATION_REMOVED, removed);
	}

	public void testTypesAddedToEmpty() {
		evaluate(new Lines(""),

		new Lines("class Test {", "}", "interface Test2 {", "}"),
	...
	public void testTypesAddedToImports() {
		...
		new Lines("import foo.Bar;", "class Test {", "}", "interface Test2 {",
				"}"),
	...
	public void testTypesAddedToPackage() {
		evaluate(new Lines("package foo;", ""),
				...
				new Lines("package foo;", "class Test {", "}",
						"interface Test2 {", "}"),
	...
	public void testTypesRemovedToEmpty() {
		evaluate(new Lines("class Test {", "}", "interface Test2 {", "}"),
	...
	public void testTypesRemovedToImports() {
		evaluate(new Lines("import foo.Bar;", "class Test {", "}",
				"interface Test2 {", "}"),
	...
	public void testTypesRemovedToPackage() {
		evaluate(new Lines("package foo;", "class Test {", "}",
				"interface Test2 {", "}"),
