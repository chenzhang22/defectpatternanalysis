...
public class ParameterComparator {
	...
	public ParameterComparator(Parameters fromParams, Parameters toParams) {
		this.fromFormalParamList = new ArrayList<ASTFormalParameter>(
				fromParams.getParameterList());
		this.unmatchedToParams = new ArrayList<ASTFormalParameter>(
				toParams.getParameterList());
	...
	public ParameterMatch getMatch(int fromIdx) {
		...
		if (bestMatch < 0) {
			return ParameterMatch.create(fromFormalParam, fromIdx, -1, -1,
					toParams);
		}
	...
	private ParameterMatch getParamMatch(int fromIdx) {
		...
		return ParameterMatch.create(fromFormalParam, fromIdx, typeMatch,
				nameMatch, toParams);
	}
