...
public class TestStaticBlock extends ItemsTest {
	...
	public void testStaticBlockRemoved() {
		...
		new FileDiffDelete(locrg(2, 5, 14), locrg(1, 1, 2, 1),
				Initializer.STATIC_BLOCK_REMOVED));
	}
	...
	public void testStaticBlockAdded() {
		...
		new FileDiffAdd(locrg(1, 1, 2, 1), locrg(2, 5, 14),
				Initializer.STATIC_BLOCK_ADDED));
	}

	public void testStaticBlockChanged() {
		evaluate(new Lines("class Test {", "    { int i; }", "}"),

		new Lines("class Test {", "    { double d; }", "}"),

		new FileDiffChange(locrg(2, 7, 11), locrg(2, 7, 14), Code.CODE_CHANGED,
				"static block"));
	}
}
