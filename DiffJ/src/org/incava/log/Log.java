...
public class Log extends ANSI {
	...
	public static boolean stack(Object whence, String msg, int startDepth,
			int numFrames) {
		StackTraceElement[] stack = (new Exception("")).getStackTrace();
		for (int si = startDepth; si < startDepth + numFrames
				&& si < stack.length; ++si) {
	...
	protected static void display(Object whence, String msg,
			StackTraceElement[] stack, int stackIndex) {
		if (verbose) {
			...
			// should be:
			// String fmt = "[%" + fileWidth + "s:%" + lineWidth + "d] {%" +
			// funcWidth + "s} %s";
			// String str = FormattedString.toString(fmt, fileName, lineNumber,
			...
			if (className.startsWith("org.") || className.startsWith("com.")) {
				className = "..."
						+ className.substring(className.indexOf('.', 5) + 1);
			}
