...
public class Line {
	...
	public Line(String message, ItemColors colors,
			StackTraceElement stackElement,
			StackTraceElement previousStackElement, Configuration config) {
		this.message = message;
		this.colors = colors;
		this.stackElements = new StackElements(stackElement,
	...
	public void appendFileName(StringBuilder sb) {
		...
		if (config.useColumns()) {
			FileName lfn = new FileName(color, stackElements,
					config.getFileWidth());
			String flstr = lfn.getFormatted();

			LineNumber lln = new LineNumber(color, stackElements,
					config.getLineWidth());
			String lnstr = lln.getFormatted();
			sb.append(flstr).append(' ').append(lnstr);
		} else {
			FileNameLineNumber lfnln = new FileNameLineNumber(color,
					stackElements, config.getFileWidth());
	...
	public void appendClassAndMethod(StringBuilder sb) {
		...
		ClassName lcn = new ClassName(colors.getClassColor(), stackElements,
				config.getClassWidth());
		sb.append(lcn.getFormatted());

		sb.append('#');

		MethodName lmn = new MethodName(colors.getMethodColor(), stackElements,
				config.getFunctionWidth());
	...
	public String getMessage() {
		Message lm = new Message(colors.getMessageColors(), stackElements,
				message);
