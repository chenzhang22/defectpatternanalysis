...
public class Type extends AccessibleElement implements Diffable<Type> {
	public static final Message TYPE_CHANGED_FROM_CLASS_TO_INTERFACE = new Message(
			"type changed from class to interface",
			ChangeType.TYPE_CHANGED_FROM_CLASS_TO_INTERFACE);
	public static final Message TYPE_CHANGED_FROM_INTERFACE_TO_CLASS = new Message(
			"type changed from interface to class",
			ChangeType.TYPE_CHANGED_FROM_INTERFACE_TO_CLASS);

	public static final Message INNER_INTERFACE_ADDED = new Message(
			"inner interface added: {0}", ChangeType.INNER_INTERFACE_ADDED);
	public static final Message INNER_INTERFACE_REMOVED = new Message(
			"inner interface removed: {0}", ChangeType.INNER_INTERFACE_REMOVED);

	public static final Message INNER_CLASS_ADDED = new Message(
			"inner class added: {0}", ChangeType.INNER_CLASS_ADDED);
	public static final Message INNER_CLASS_REMOVED = new Message(
			"inner class removed: {0}", ChangeType.INNER_CLASS_REMOVED);
	...
	public void diff(Type toType, Differences differences) {
		if (!isInterface() && toType.isInterface()) {
			differences.changed(this, toType,
	...
	public <ItemType extends SimpleNode> List<ItemType> getDeclarationsOfClass(
			Class<ItemType> cls) {
