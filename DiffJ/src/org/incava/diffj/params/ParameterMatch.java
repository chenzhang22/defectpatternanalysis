...
public abstract class ParameterMatch {
	public enum StatusType {
		NAME_CHANGED, TYPE_CHANGED, REORDERED, REMOVED, ADDED
	};

	public static ParameterMatch create(ASTFormalParameter fromFormalParam,
			int index, int typeMatch, int nameMatch, Parameters toParams) {
		if (typeMatch == index) {
			if (nameMatch == index) {
				return new ParameterExactMatch(fromFormalParam, index,
						typeMatch, nameMatch, toParams);
			} else {
				return new ParameterExactTypeMatch(fromFormalParam, index,
						typeMatch, nameMatch, toParams);
			}
		} else if (nameMatch == index) {
			return new ParameterExactNameMatch(fromFormalParam, index,
					typeMatch, nameMatch, toParams);
		} else if (typeMatch >= 0) {
			return new ParameterTypeMatch(fromFormalParam, index, typeMatch,
					nameMatch, toParams);
		} else if (nameMatch >= 0) {
			return new ParameterNameMatch(fromFormalParam, index, typeMatch,
					nameMatch, toParams);
		}
		return new ParameterNoMatch(fromFormalParam, index, typeMatch,
				nameMatch, toParams);
	}
	...
	public ParameterMatch(ASTFormalParameter fromFormalParam, int index,
			int typeMatch, int nameMatch, Parameters toParams) {
