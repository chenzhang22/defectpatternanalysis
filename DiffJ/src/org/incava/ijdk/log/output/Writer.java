...
import static org.incava.ijdk.util.IUtil.isEmpty;
import static org.incava.ijdk.util.IUtil.isTrue;
 ...
 * thorough interfaces than this class.
 * </p>
 * 
...
public class Writer {
	...
	private PrintWriter out = new PrintWriter(new PrintStream(
			new FileOutputStream(FileDescriptor.out)), true);

	private List<String> packagesSkipped = list("org.incava.ijdk.log",
			"org.incava.qualog");
	...
	public void setMethodColor(String className, String methodName,
			ANSIColor color) {
	...
	public void set(boolean columns, int fileWidth, int lineWidth,
			int classWidth, int functionWidth) {
	...
	public boolean stack(Level level, ItemColors colors, String name,
			Object obj, int numFrames) {
		if (!isLoggable(level)) {
			return true;
		}

		LogElement le = LogElementFactory.create(level, colors, name, obj,
				numFrames);
	...
	public boolean isSkipped(StackTraceElement ste) {
		...
		if (classesSkipped.contains(className)
				|| methodsSkipped.contains(ste.getMethodName())) {
			return true;
		}
	...
	public ANSIColor getFileColor(ItemColors elmtColors,
			StackTraceElement stackElement) {
		return or(elmtColors.getFileColor(),
				config.getFileColor(stackElement.getFileName()));
	}

	public ANSIColor getClassColor(ItemColors elmtColors,
			StackTraceElement stackElement) {
		return or(elmtColors.getClassColor(),
				config.getClassColor(stackElement.getClassName()));
	}

	public ANSIColor getMethodColor(ItemColors elmtColors,
			StackTraceElement stackElement) {
		return or(
				elmtColors.getMethodColor(),
				config.getMethodColor(stackElement.getClassName(),
	...
	public synchronized boolean stack(LogElement le) {
		...
		int numFrames = outputType.equals(OutputType.QUIET) ? 1 : le
				.getNumFrames();
		...
		for (int framesShown = 0; fi < stack.length && framesShown < numFrames; ++fi, ++framesShown) {
			...
			ItemColors lineColors = new ItemColors(msgColors, getFileColor(
					elmtColors, stackElement), getClassColor(elmtColors,
					stackElement), getMethodColor(elmtColors, stackElement));

			Line line = new Line(le.getMessage(), lineColors, stackElement,
					prevStackElement, config);
			out.println(line.getLine(framesShown > 0,
					outputType.equals(OutputType.VERBOSE)));
			prevStackElement = stackElement;
		}
		return true;
	}

	private ANSIColorList getMessageColors(ItemColors elmtColors,
		...
		if (isEmpty(msgColors)) {
			ANSIColor col = or(
					config.getMethodColor(ste.getClassName(),
							ste.getMethodName()),
