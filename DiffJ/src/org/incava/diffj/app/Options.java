...
public class Options extends OptionSet {
	...
	private boolean briefOutput = true;
	private boolean contextOutput = false;
	private boolean highlightOutput = true;
	...
	public Options() {
		...
		contextOpt = addBooleanOption("context",
				"Show context (non-brief form only)");
		highlightOpt = addBooleanOption("highlight",
				"Whether to use colors (context output only)");
		tabWidthOpt = addOption(new IntegerOption("tabwidth",
				"The number of spaces to treat tabs equal to"));
		recurseOpt = addOption(new BooleanOption("recurse",
				"Process directories recursively", 'r'));
		verboseOpt = addBooleanOption("verbose",
				"Whether to run in verbose mode (for debugging)");
		versionOpt = addOption(new BooleanOption("version",
				"Displays the version", 'v'));

		String javaVersions = StringExt.join(
				Arrays.asList(new String[] { Java.SOURCE_1_3, Java.SOURCE_1_4,
						Java.SOURCE_1_5 + " (the default)",
						"or " + Java.SOURCE_1_6, }), ", ");

		fromSourceOpt = addOption(new StringOption("from-source",
				"The Java source version, of the from-file; " + javaVersions));
		toSourceOpt = addOption(new StringOption("to-source",
				"The Java source version, of the to-file; " + javaVersions));
		sourceOpt = addOption(new StringOption("source",
				"The Java source version of both from-file and the to-file"));

		// processed as a valid option, but unused.
		addOption(new BooleanOption(
				"unified",
				"Output unified context. Unused; for compatibility with GNU diff",
				'u'));

		// svn diff --diff-cmd cmd passes
		// "-u, -L first, -L second, file1, file2":
		StringOption nameOpt = new StringOption("name",
				"Sets the first/second name to be displayed", 'L') {
		...
		BooleanOption fmtOpt = new BooleanOption("format",
				"Sets the format to unified diff; ignored");
		addOption(fmtOpt);
