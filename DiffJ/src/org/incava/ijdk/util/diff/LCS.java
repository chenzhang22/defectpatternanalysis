...
public class LCS<ObjectType> {
	...
	public List<Integer> getMatches() {
		...
		while (fromStart <= fromEnd && toStart <= toEnd
				&& equals(comparator, from.get(fromStart), to.get(toStart))) {
			matches.put(fromStart++, toStart++);
		}

		while (fromStart <= fromEnd && toStart <= toEnd
				&& equals(comparator, from.get(fromEnd), to.get(toEnd))) {
	...
	public void addMatches(TreeMap<Integer, Integer> matches, int fromStart,
			int fromEnd, int toStart, int toEnd) {
		...
		for (int idx = fromStart; idx <= fromEnd; ++idx) {
			...
			Integer k = 0;
			ListIterator<Integer> pit = positions
	 ...
	 * constructor, if any.
	 */
	protected boolean equals(Comparator<ObjectType> comparator, ObjectType x,
			ObjectType y) {
