...
public class JavaFile extends JavaFSElement {
	...
	public static JavaFile createFile(File dir, JavaFSElement otherElmt)
			throws DiffJException {
		tr.Ace.onRed("dir: " + dir + "; otherElmt: " + otherElmt);
		try {
			JavaElementFactory jef = new JavaElementFactory();
			return jef.createFile(new File(dir, otherElmt.getName()), null,
					otherElmt.getSourceVersion());
	...
	public static int compare(Report report, JavaFile fromFile, JavaFile toFile)
			throws DiffJException {
	...
	protected JavaFile(File file, String label, String contents,
			String sourceVersion) throws DiffJException {
		...
		try {
			...
			if (contents != null) {
				...
				FileReader reader = isStdin ? new FileReader(FileDescriptor.in)
						: new FileReader(file);
				this.contents = ReaderExt.readAsString(reader,
						EnumSet.of(ReadOptionType.ADD_EOLNS));
			}
			this.label = label == null ? (isStdin ? "-" : file.getPath())
					: label;
		} catch (FileNotFoundException e) {
			throw new DiffJException("Error opening file '"
	...
	public JavaFile(File file, String label, String sourceVersion)
			throws DiffJException {
	...
	protected JavaParser getParser() throws DiffJException {
		...
		if (sourceVersion.equals(Java.SOURCE_1_3)) {
			parser.setJDK13();
		} else if (sourceVersion.equals(Java.SOURCE_1_5)
				|| sourceVersion.equals(Java.SOURCE_1_6)) {
			// no setJDK16 yet in PMD
			parser.setJDK15();
		} else if (!sourceVersion.equals(Java.SOURCE_1_4)) {
			throw new DiffJException("source version '" + sourceVersion
					+ "' not recognized");
		}
	...
	public CompilationUnit compile() throws DiffJException {
		try {
			...
			throw new DiffJException("Error tokenizing " + label + ": "
					+ tme.getMessage(), tme);
		} catch (ParseException pe) {
			throw new DiffJException("Error parsing " + label + ": "
					+ pe.getMessage(), pe);
		} catch (Exception e) {
			e.printStackTrace(System.err);
			throw new DiffJException("Error processing " + label + ": "
					+ e.getMessage(), e);
		}
	}

	public int compareTo(Report report, JavaFSElement toElmt)
			throws DiffJException {
		return toElmt.compareFrom(report, this);
	}

	public int compareFrom(Report report, JavaFile fromFile)
			throws DiffJException {
		return compare(report, fromFile, this);
	}

	public int compareFrom(Report report, JavaDirectory fromDir)
			throws DiffJException {
	...
	public int compare(Report report, JavaFile toFile) throws DiffJException {
		try {
			...
			report.reset(getLabel(), getContents(), toFile.getLabel(),
					toFile.getContents());
