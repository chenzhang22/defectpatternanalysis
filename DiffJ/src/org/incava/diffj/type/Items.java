...
public abstract class Items<DiffJType extends Diffable<DiffJType>, PmdAstType extends SimpleNode> {
	...
	public void diff(Items<DiffJType, PmdAstType> toItems,
	...
	public void addAdded(List<DiffJType> added, Differences differences) {
		for (DiffJType toAdd : added) {
			...
			differences.added(type.getNode(), toAdd.getNode(),
					toAdd.getAddedMessage(), name);
		}
	}

	public void addRemoved(List<DiffJType> removed, Type toType,
			Differences differences) {
		for (DiffJType goner : removed) {
			String name = goner.getName();
			differences.deleted(goner.getNode(), toType.getNode(),
					goner.getRemovedMessage(), name);
		}
	}
}
