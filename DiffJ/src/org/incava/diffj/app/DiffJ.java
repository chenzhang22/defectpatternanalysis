...
public class DiffJ {
	...
	public DiffJ(boolean briefOutput, boolean contextOutput,
			boolean highlightOutput, boolean recurseDirectories,
			String fromLabel, String fromSource, String toLabel, String toSource) {
		...
		this.report = // briefOutput ? new BriefReport(writer) :
		new DetailedReport(writer, contextOutput, highlightOutput);
	...
	protected JavaFSElement getJavaElement(File file, String label,
			String source) {
		try {
	...
	public boolean compareElements(JavaFSElement fromElmt, JavaFSElement toElmt) {
		try {
			if (fromElmt == null) {
				return false;
			}
	...
	public void processNames(List<String> names) {
		...
		for (int ni = 0; ni < names.size() - 1; ++ni) {
			JavaFSElement fromElmt = getFromElement(names.get(ni));
			if (!compareElements(fromElmt, toElmt)) {
				break;
			}
		}
	...
	public static void main(String[] args) {
		...
		System.out.println("diffj:" + diffj.fileDiffs);
