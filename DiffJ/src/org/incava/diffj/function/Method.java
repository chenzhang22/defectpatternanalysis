...
public class Method extends Function implements Diffable<Method> {
	public static final Message METHOD_REMOVED = new Message(
			"method removed: {0}", ChangeType.METHOD_REMOVED);
	public static final Message METHOD_ADDED = new Message("method added: {0}",
			ChangeType.METHOD_ADDED);
	public final static Messages METHOD_MSGS = new Messages(METHOD_ADDED, null,
			METHOD_REMOVED);

	public static final Message RETURN_TYPE_CHANGED = new Message(
			"return type changed from {0} to {1}",
			ChangeType.RETURN_TYPE_CHANGED);
	public static final Message METHOD_BLOCK_ADDED = new Message(
			"method block added", ChangeType.METHOD_BLOCK_ADDED);
	public static final Message METHOD_BLOCK_REMOVED = new Message(
			"method block removed", ChangeType.METHOD_BLOCK_REMOVED);
	...
	protected void compareReturnTypes(Method toMethod, Differences differences) {
		...
		if (!fromRetTypeStr.equals(toRetTypeStr)) {
			differences.changed(fromRetType, toRetType, RETURN_TYPE_CHANGED,
					fromRetTypeStr, toRetTypeStr);
		}
	}
