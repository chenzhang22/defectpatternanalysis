...
public class Parameters {
	public static final Message PARAMETER_REMOVED = new Message(
			"parameter removed: {0}", ChangeType.PARAMETER_REMOVED);
	public static final Message PARAMETER_ADDED = new Message(
			"parameter added: {0}", ChangeType.PARAMETER_ADDED);
	public static final Message PARAMETER_REORDERED = new Message(
			"parameter {0} reordered from argument {1} to {2}",
			ChangeType.PARAMETER_REORDERED);
	public static final Message PARAMETER_TYPE_CHANGED = new Message(
			"parameter type changed from {0} to {1}",
			ChangeType.PARAMETER_TYPE_CHANGED);
	public static final Message PARAMETER_NAME_CHANGED = new Message(
			"parameter name changed from {0} to {1}",
			ChangeType.PARAMETER_NAME_CHANGED);
	public static final Message PARAMETER_REORDERED_AND_RENAMED = new Message(
			"parameter {0} reordered from argument {1} to {2} and renamed {3}",
			ChangeType.PARAMETER_REORDERED_AND_RENAMED);
	...
	protected void markParametersAdded(Parameters toParams,
	...
	protected void markParametersRemoved(Parameters toParams,
			Differences differences) {
		List<Token> names = getParameterNames();
		for (Token name : names) {
			differences.changed(name, toParams.params, PARAMETER_REMOVED,
					name.image);
		}
	}

	public void compareEachParameter(Parameters toParams,
			Differences differences) {
		...
		List<ASTFormalParameter> unmatchedParams = pc
				.getUnmatchedToParameters();
		for (ASTFormalParameter unmatchedParam : unmatchedParams) {
			...
			differences.changed(params, unmatchedParam, PARAMETER_ADDED,
					toName.image);
		}
	}
	...
	public double getMatchScore(Parameters toParams) {
		...
		ParameterTypes toTypes = new ParameterTypes(
				toParams.getParameterTypes());
