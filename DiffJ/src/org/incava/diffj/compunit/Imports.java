...
public class Imports {
	public static final Message IMPORT_REMOVED = new Message(
			"import removed: {0}", ChangeType.IMPORT_REMOVED);
	public static final Message IMPORT_ADDED = new Message("import added: {0}",
			ChangeType.IMPORT_ADDED);
	public static final Message IMPORT_SECTION_REMOVED = new Message(
			"import section removed", ChangeType.IMPORT_SECTION_REMOVED);
	public static final Message IMPORT_SECTION_ADDED = new Message(
			"import section added", ChangeType.IMPORT_SECTION_ADDED);
	...
	public Imports(ASTCompilationUnit compUnit) {
		...
		List<ASTImportDeclaration> astImports = CompilationUnitUtil
				.getImports(compUnit);
	...
	protected void compareEach(Imports toImports, Differences differences) {
		...
		for (String name : names) {
			...
			if (fromImp == null) {
				differences.added(getFirstDeclaration(), toImp, IMPORT_ADDED,
						name);
			} else if (toImp == null) {
				differences.deleted(fromImp, toImports.getFirstDeclaration(),
						IMPORT_REMOVED, name);
			}
		}
	}
	...
	protected void markAllAdded(Imports toImports, Differences differences) {
		...
		differences.added(fromStart, fromEnd, toStart, toEnd,
	...
	protected void markAllRemoved(Imports toImports, Differences differences) {
		...
		Token toEnd = toStart;
		differences.deleted(fromStart, fromEnd, toStart, toEnd,
				IMPORT_SECTION_REMOVED);
	}

	protected Token getFirstTypeToken() {
		List<ASTTypeDeclaration> types = CompilationUnitUtil
				.getTypeDeclarations(compUnit);
