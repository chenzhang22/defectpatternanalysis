...
public abstract class Modifiers {
	public static final Message MODIFIER_REMOVED = new Message(
			"modifier removed: {0}", ChangeType.MODIFIER_REMOVED);
	public static final Message MODIFIER_ADDED = new Message(
			"modifier added: {0}", ChangeType.MODIFIER_ADDED);
	public static final Message MODIFIER_CHANGED = new Message(
			"modifier changed from {0} to {1}", ChangeType.MODIFIER_CHANGED);
	...
	public void diff(Modifiers toModifiers, Differences differences) {
		...
		for (int modType : modifierTypes) {
			...
			if (fromMod == null) {
				if (toMod != null) {
					differences.changed(node.getFirstToken(), toMod,
							MODIFIER_ADDED, toMod.image);
				}
			} else if (toMod == null) {
				differences.changed(fromMod, toModifiers.node.getFirstToken(),
						MODIFIER_REMOVED, fromMod.image);
			}
		}
	}
